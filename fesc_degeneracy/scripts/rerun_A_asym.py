...
NOTE: the xHI(z) data vector below is a REPRESENTATIVE STAND-IN (literature-
motivated, 8 points z=6.5-11). Swap in the exact vector used in the Letter
before quoting numbers in the response. Differential conclusions (CDM vs SIDM
after profiling) are insensitive to this choice; absolute chi2 is not.
"""
import numpy as np
...
XHI_DATA=[  # (z, xHI, sig_up, sig_dn, kind) asymmetric
 (6.5,0.33,0.18,0.27,'two'),
 (7.08,0.40,0.21,0.19,'two'),
 (7.12,0.53,0.18,0.47,'two'),
 (7.44,0.65,0.27,0.34,'two'),
 (7.9,0.76,0.30,0.30,'lower'),
 (8.28,0.91,0.09,0.22,'two'),
 (9.3,0.64,0.17,0.23,'two'),
 (9.91,0.92,0.08,0.10,'two')]
...
def reion_chi2_scan(shape, fesc_grid):
    ...
    for (zd,xo,su,sd,kind) in XHI_DATA:
        i0=int(np.argmin(np.abs(ZGRID-zd)))
        resid=(1-Qtraj[i0])-xo
        sg=np.where(resid>0,su,sd)
        pen=(resid/sg)**2
